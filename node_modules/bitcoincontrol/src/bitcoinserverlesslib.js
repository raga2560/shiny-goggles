var compositekeylib = require('./bitcoincompositekeylib');
...
function bitcoinInit(contractinfo, partnerinfo, network, url) 
{
...
function getbitcoinbalance (param)
{

var url = globalurl + '/v1/btc/test3/addrs/';
...
txout.index = j;
txout.confirmations = globaltxs[i].confirmations;
...
function regularBitcoinSendingFund(type, amount, targetaddr, activatingkeypair)
{
...
var activatepromise = new Promise(function (resolve, reject) {
    ...
    var promise = getbitcoinbalance(spendingaddr );
   ...
   var status = {
   reason: "No transactions with minimum confirmation available to spend",
   status: -1,
   error: '',
   data: '',
   };
 ...
 // Only first in array is used, later we can change by choosing best transaction to pick
    ...
    signinglist.push(spendoutlist[i]);
       }
    }
...
function getBitcoinCompositeAddress(senderstub, uidkey, usagetype  )
{
...
// type 2, hash of hashofdoc is used in  string
  ...
  var Pinkey = Buffer.from(Pin);
   ...
   var docaddr = compositekeylib.getBitcoinBufControlCodeAddress(Pinkey, 
		uidkey,
		globalnetwork);
...
var activatepromise = new Promise(function (resolve, reject) {
     var promise = getbitcoinbalance(docaddr );
       ...
       if(globalbalance < 100)
    {
        console.log("globalbalance="+globalbalance);
   var status = {
   reason: " No sufficient balance to spend in :"+ docaddr,
   status: -1,
   error: '',
   data: '',
   };
 ...
 var status = {
   reason: "Transaction build failed",
   status: -1,
   error: '',
   data: '',
   };
...
function compBitcoinReceive1toManyFund(senderstub, uidkey, targetaddr )
{
// type 1, hashofdoc is used in raw string
// type 2, hash of hashofdoc is used in  string
...
var activatepromise = new Promise(function (resolve, reject) {
 ...
 if(globalbalance < 100)
   ...
   for(var i =0; i< spendoutlist.length ; i++ ) {
     if(spendoutlist[i].confirmations < 3) {
 var status = {
   reason: "Less than minimum confirmations to spend",
   status: -1,
   error: '',
   data: '',
   };
...
// var url = globalurl + '/v1/btc/test3/addrs/';
   var lurl = globalurl + '/v1/btc/test3/txs/push';
   var promise = new Promise(function (resolve, reject) {
  ...
  console.log("body:"+body);
        if (error) {
 var status = {
   reason: "Broadcast transaction failed",
   status: -1,
   error: error,
   data: '',
       };
 ...
 var status = {
   reason: "Transaction broadcast success",
   status: 0,
   error: '',
   data: bodycheck,
   };
...
module.exports = {
   bitcoinInit: bitcoinInit,
   regularBitcoinSendingFund: regularBitcoinSendingFund,
   getBitcoinCompositeAddress: getBitcoinCompositeAddress,
   doc1Checktx: doc1Checktx,
   doc1CheckAddr: doc1CheckAddr,
   compReceiveFund: compReceiveFund,
   compBitcoinReceive1toManyFund: compBitcoinReceive1toManyFund,
   doc2Uploadv: doc2Uploadv,
   sendBitcointx: sendBitcointx,
